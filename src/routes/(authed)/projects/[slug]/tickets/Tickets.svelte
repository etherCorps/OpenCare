...
<div class="space-y-4">
    <div class="mx-auto flex flex-row items-center justify-between">
        <div>
            <h3 class="text-xl font-semibold text-primary-600">{$currentProject.name} Tickets</h3>
        </div>
        <div>
            {#if adminPermissions}
                <button
                        ...
                        type="button"
                        class="bg-primary-600 text-primary-100 focus:bg-secondary inline-flex w-full items-center justify-center rounded-lg border px-4 py-3 text-sm  font-semibold leading-5 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 "
                >
                   Add Ticket
                </button>
            {/if}
